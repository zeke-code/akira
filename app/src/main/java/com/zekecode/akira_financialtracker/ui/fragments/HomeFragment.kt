...
class HomeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.usedBudgetPercentage.observe(viewLifecycleOwner) { usedBudget ->
            when {
                usedBudget < 0 -> binding.homeUsedBudgetText.text = getString(R.string.home_no_budget_used_text)
                usedBudget < 100 -> binding.homeUsedBudgetText.text = getString(R.string.home_used_budget_text, usedBudget)
                usedBudget >= 100 -> binding.homeUsedBudgetText.text = getString(R.string.home_all_budged_used_text)
            }
